...
var (
	...
	MiningBlockReward			  = big.NewInt(10)    //50 DTC reward for one block at first
	RewardHalvingInterval         = big.NewInt(6000000)  //about 625 days for each reward halving
...
func calcDifficultyHomestead(time uint64, parent *types.Header) *big.Int {
	...
	// algorithm:
	// diff = (parent_diff +
	//         (parent_diff / 2048 * max(1 - (block_timestamp - parent_timestamp) // 9, -99))
	//        ) + 2^(periodCount - 2)
	...
	// (parent_diff + parent_diff // 2048 * max(1 - (block_timestamp - parent_timestamp) // 9, -99))
	y.Div(parent.Difficulty, params.DifficultyBoundDivisor)
	x.Mul(y, x)
	x.Add(parent.Difficulty, x)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	blockReward := new(big.Int).Mul(FrontierBlockReward, MiningBlockReward)
	...
	reward := new(big.Int).Div(blockReward, big.NewInt(1e+18))
	log.Info("mining block coinbase", "halvings", halvings, "reward", reward.Uint64(), "difficulty", header.Difficulty.Uint64())
